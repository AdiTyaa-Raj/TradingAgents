...
def _llm_provider_table() -> list[tuple[str, str, str | None]]:
    ...
    return [
        ...
        ("Claude Code CLI - uses your signed-in Claude plan, no API key", "claude_code", None),
...
def confirm_claude_code_cli() -> None:
    """Check the Claude Code CLI is installed before the run starts.

    This provider has no API key to prompt for — the run is authenticated by
    the CLI's own signed-in session — so the equivalent pre-flight is proving
    the binary exists. A missing sign-in can't be detected without spending a
    turn, so it's only mentioned as a reminder; the first LLM call surfaces it
    with the `claude login` hint if it's actually missing.
    """
    ...
    path = find_cli()
    if path is None:
        console.print(f"\n[red]{INSTALL_HINT}[/red]")
        exit(1)

    version = cli_version() or "unknown version"
    console.print(f"[green]✓ Using Claude Code CLI at {path} ({version})[/green]")
    console.print(
        "[green]  Authenticated by your signed-in Claude plan — no API key "
        "needed. Run [/green][cyan]claude[/cyan][green] and sign in if the run "
        "reports an auth error.[/green]"
    )
